...
export default class Editor {
  ...
  static parseHtmlStructureText(structText) {
    const container = [];
    const structure = structText.replace(/\d+/g, (match) => 'i'.repeat(+match));
  ...
  get _classList() {
    ...
    const containers =
      containerCount > 1
        ? [...Array(containerCount)].map((_, index) => `container${index + 1}`)
        : ['container', 'item'];
  ...
  _createSelectorCodeLine(line, codeLine, selectorIndex) {
    ...
    Tag.appendChildren(codeLine, [deleteButton, '.', selectorSpan, '\u00A0{']);
  }

  _createPropCodeLine(prop, value, codeLine, selectorIndex, propIndex) {
    const propSpan = this._createPropCodeButton(prop, selectorIndex, propIndex);
    ...
    const deleteButton = this._createDeleteCodeButton(selectorIndex, propIndex);
  ...
  _createPropCodeLines(prop, valueArr, numberIndex, selectorIndex, propIndex) {
    const output = [];
    let inputIndex = 0;
          ...
          case 'ArrowUp':
            {
              ...
              nextElem.focus();
              nextElem.selectionStart = nextElem.selectionEnd = selectionStart;
            }
            break;
  ...
  _textareaKeydownEventListener(e, numbers) {
    ...
    switch (key) {
      case 'Enter':
        {
          ...
          if (value[openingIndex - 1] === '{' && value[closingIndex] === '}') {
            e.preventDefault();
          ...
          while (true) {
            ...
            if (value[closingIndex] === '{' || closingIndex === value.length) {
              this._updateLineNumbers(currentTarget, numbers);
              return;
            }
          }
      ...
      case '{':
        ...
        currentTarget.selectionStart = currentTarget.selectionEnd = startIndex;
        break;
    }
  ...
  _textTableKeydownEventListener() {
    ...
    const startIndex =
      firstLineChildNodes.reduce(
        (acc, node, i) => (i < nodeIndex ? acc + node.textContent.length : acc),
        0
      ) + offset;
